<template>
  <div id="goods">
    ...
    <div class="content3">
      ...
      <el-button type="primary" icon="el-icon-circle-plus-outline" @click="showAdd = true">新增班级</el-button>
      <el-dialog title="新增班级" :visible.sync="showAdd">
        <el-form :model="addClass">
          <el-form-item label="年级" width="120px">
            <el-input v-model="addClass.grade" autocomplete="off"></el-input>
          </el-form-item>
          <el-form-item label="班级" width="60px">
            <el-input v-model="addClass.class" autocomplete="off"></el-input>
          </el-form-item>
        </el-form>
  ...
  export default{
    ...
    methods:{
      sureAdd(e){
        var that = this;
        console.log(e)
        console.log(that.addClass)
        if(that.addClass.grade>6){
          that.$message({
            type: 'info',
            message: '小学最高为六年级哦'
          });
        }else if(that.addClass.class == "" || that.addClass.grade == ""){
          that.$message({
            type: 'info',
            message: '不能为空'
          });
          ...
          postId.append('class_grade',that.addClass.grade)
          postId.append('class_class',that.addClass.class)
          $.ajax({
            url:GLOBAL.baseURL+'/class/add',
            type:'post',
            xhrFields:{
      ...
      handleClick(e){
        ...
        postId.append('class_id',e.class_id)
        $.ajax({
          url:GLOBAL.baseURL+'/class/checkT',
          type:'post',
          xhrFields:{
            withCredentials:false
          },
